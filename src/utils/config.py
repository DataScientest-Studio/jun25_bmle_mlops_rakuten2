...
try:
    ...
    except ImportError:  # pragma: no cover - cas d'environnement mal configuré
        raise ImportError(
            "tomli ou tomllib requis pour lire les fichiers TOML. "
            "Installez avec: pip install tomli"
        )
...
class Config:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        """
        self.config_path: Path = self._find_config(config_path)
    ...
    def _find_config(self, config_path: Optional[str]) -> Path:
        ...
        Returns:
            Chemin absolu vers config.toml
        ...
        for candidate in candidates:
            if candidate.exists():
                ...
                logger.info("Configuration trouvée: %s", candidate)
                return candidate

        raise FileNotFoundError(
            "config.toml non trouvé. "
            "Placez-le dans config/ ou spécifiez le chemin à Config(...)."
        )
    ...
    def load(self) -> None:
        ...
        Raises:
            ...
            tomllib.TOMLDecodeError: Si le fichier TOML est mal formaté
        """
        logger.info("Chargement de la configuration depuis %s", self.config_path)
    ...
    def random_seed(self) -> int:
        ...
        return int(self._config.get("random", {}).get("seed", 42))
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\nSeed aléatoire: {cfg.random_seed}")
        print(f"Max features texte: {cfg.get('text.max_features', 'non défini')}")
        print(f"Taille images: {cfg.get('images.size', 'non défini')}")
        print(f"Modèle: {cfg.get('model.name', 'non défini')}")
